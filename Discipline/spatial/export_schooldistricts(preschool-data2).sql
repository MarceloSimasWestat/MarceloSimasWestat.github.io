...
SELECT schooldistricts.gid, schooldistricts.geom,
	...
	CASE WHEN pk_pct::NUMERIC < 0.001
		THEN round(pk_pct::NUMERIC * 100, 2)::TEXT || '%'
		ELSE round(pk_pct::NUMERIC * 100, 1)::TEXT || '%'
	END AS "e",
	COALESCE(pk_pctc, '-1') AS "f",
...
INNER JOIN pk_pctc_data ON schooldistricts.leaid = pk_pctc_data.leaid::TEXT;
