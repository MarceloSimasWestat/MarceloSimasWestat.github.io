	...
	function chart(selection) {
		...
		if (notes) {
			d3.select("#"+ sectionID).append("div")
				.attr("id", "notes" + chartID)
				.html(function() {
					if (toggles === 1) { return "<span class = 'chartNotes'>" + notes[0] + "</span>"; }
					else { return "<span class = 'chartNotes'>" + notes + "</span>"; };
				});
		};
		...
		if (source) {
			d3.select("#"+ sectionID).append("div")
				...
				.html(function() {
					if (toggles === 1) { return "<span class = 'chartNotes'>" + source[0] + "</span>"; }
					else { return "<span class = 'chartNotes'>" + source + "</span>"; };
				});
		};
		...
		function updateData() {
			...
			d3.select("#notes" + chartID)
				.html("<span class = 'chartNotes'>" + notes + "</span>");

			d3.select("#source" + chartID)
				.html("<span class = 'chartNotes'>" + source + "</span>");
...
// multi-line chart v2
// includes state lines, NO state names

function multi_line_v2() {
	...
	var	width = [],
		...
		altText = "Fill in alt text for screen readers!",
		group = "",
		yAxisLabel = "DEFINE Y AXIS LABEL",
		notes,
		source,
		containerID = [],
		subcontainerID = [],
	...
	function chart(selection) {
		...
		data.forEach(function(d) {
			...
			if (d.all_p === "†" || d.all_p === "-") { d.all_p === ""; }
			if (d.el_p === "†" || d.el_p === "-") { d.el_p === ""; }
		...
		var xScale = d3.scale.ordinal().rangeBands([0, widthAdj]),
				yScale = d3.scale.linear().range([heightAdj, 0]);
		...
		var highlighted_state,
				seleted_state;

		var line_values = d3.svg.line()
			.defined(function(d) { return !isNaN(d.el_p); })
			.x(function(d) { return xScale(d.year) + xScale.rangeBand()/2; })
			.y(function(d) { return yScale(d.el_p); });

		// create line group

		var line_group = svg.selectAll("g.line_group")
		...
		// add dots, initially only for national

		line_group.selectAll(".dot")
			.data(function(d) { return d.values.filter(function(d) { return !isNaN(d.el_p); }); })
			.enter()
				.append("circle")
				.attr("class", "dot")
				.classed("state", function(d) {
		...
		line_group.selectAll(".dot_text")
			.data(function(d) { return d.values.filter(function(d) { return d.year === years_latest; }); })
			.enter()
				.append("text")
					.attr("class", "dot_text")
					.classed("state", function(d) {
		...
		line_group.selectAll(".dot_text.state")
			.style("fill", "#778899");

		// hover highlight function
		// should only apply to non-clicked states
			...
			line_group.select(".line.state:not(.highlighted)")
				.transition()
					...
					.style("stroke-width", function(d) {
						if (d.key === highlighted_state) { return 2; }
						else { return 1 };
					})
					.style("opacity", function(d) {
						if (d.key === highlighted_state) { return 1; }
						else { return 0.25; };
					});

			line_group.selectAll(".dot_text.state:not(.highlighted)")
				.style("pointer-events", function(d) {
					if (d.state === highlighted_state) { return "auto"; }
					else { return "none"; };
				})
				.transition()
					.duration(animateTime/3)
					...
					.style("opacity", function(d) {
						if (d.state === highlighted_state) { return 1; }
						else { return 0; };
					});
		...
		function highlight() {
				...
				line_group.select(".line.state")
					...
					.transition()
						...
						.style("stroke-width", function(d) {
							if (d.key === selected_state) { return 2; }
							else { return 1 };
						})
						.style("opacity", function(d) {
							if (d.key === selected_state) { return 1; }
							else { return 0.25; };
						});
				...
				line_group.selectAll(".dot.state")
					...
					.transition()
						...
						.style("opacity", function(d) {
							if (d.state === selected_state) { return 1; }
							else { return 0; };
						});
...
function hex_map() {
	...
	function chart(selection) {
		...
		// these may need to be tweaked; if the actual numbers are needed apply the same kind of transformations
